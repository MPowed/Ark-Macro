...
oSpinButton  = keyboard.Key.page_up
...
def printScreen(food):
    ...
    print('Pg Up - O spin metal')


def oSpin(what):
    ok = windll.user32.BlockInput(True)
    mouse.position = (1470, 250)
    mouse.click(Button.left)
    time.sleep(.1)
    mouse.position = (275, 240)
    mouse.click(Button.left)
    for k in range(len(what)):
        kybrd.press(Key.backspace)

    kybrd.release(Key.backspace)
    kybrd.type(what)
    time.sleep(.1)
    mouse.position = (165, 330)
    mouse.click(Button.left)
    kybrd.type('o')
    time.sleep(.1)
    mouse.position = (250, 330)
    kybrd.type('o')
    time.sleep(.1)
    mouse.position = (165, 330)
    kybrd.type('o')
...
def on_press(key):
    ...
    if key == mouseClickStartStop:
        ...
        if feed == True:
            ...
            printSpace()
    elif key == foodButton and keyboard.Key.shift not in current_keys:
            if feed == True:
                feeding(food[foodSet])
    elif key == oSpinButton and keyboard.Key.shift not in current_keys:
